...
export class BoardRewardTracker {
    ...
    registerDirectPlacement(word: string, player: Player, wordPoints?: number) {
        this.wordsTracker.registerDirectPlacement(word, player.username);
        this.checkRewards(word, player, wordPoints);
    }
    registerFormedWord(word: string, player: Player, wordPoints?: number) {
        this.wordsTracker.registerWordFormed(word, player.username);
        this.checkRewards(word, player, wordPoints);
    }
    ...
    checkRewards(word: string, player: Player, wordPoints?: number) {
        ...
        if (this.checkThreeWordsAtOnce(player.username)) Matcher.notifyGoalManager(player, GoalTitle.ThreeWordsAtOnce);
    ...
    checkThirdTimeCharm(player: Player) {
        const directPlacements = this.wordsTracker.getDirectPlacements(player.username);
