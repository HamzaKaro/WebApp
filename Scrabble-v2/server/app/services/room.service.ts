...
export class RoomService {
    ...
    removeRoom(roomName: string) {
        ...
        if (roomAvailableToRemove) {
            roomAvailableToRemove.removePlayers();
            roomAvailableToRemove.elapsedTime = -1;
            roomAvailableToRemove.isGameOver = true;
            // clearInterval(roomAvailableToRemove.timerId);
            this.roomsAvailable.splice(this.roomsAvailable.indexOf(roomAvailableToRemove), 1);
        }
        if (roomUnavailableToRemove) {
            roomUnavailableToRemove.removePlayers();
            roomUnavailableToRemove.elapsedTime = -1;
            roomUnavailableToRemove.isGameOver = true;
            // clearInterval(roomUnavailableToRemove.timerId);
    ...
    private convertToServerRoom(clientRoom: Room): Room {
        ...
        serverRoom.roomInfo.nbHumans = clientRoom.roomInfo.nbHumans;
        return serverRoom;
    }
}
