...
export class HttpService {
    ...
    constructor(private http: HttpClient) {
        ...
        this.avatarBaseUrl = 'dictionaries';
    ...
    getUserCoins(): Observable<number> {
        this.clearError();
        const url = `${this.baseUrl}/user/user-coins`;
        return this.http.get<number>(url, { headers: this.createCacheHeaders() }).pipe(catchError(this.handleError<number>('getUserCoins')));
    }
    ...
    addAvatar(avatar: Avatar): Observable<HttpResponse<Avatar>> {
        this.clearError();
        const dictionariesUrl = `${this.baseUrl}/${this.avatarBaseUrl}/${encodeURIComponent(avatar.name)}`;
        return this.http
            .post<Avatar>(dictionariesUrl, avatar, { headers: this.createCacheHeaders(), observe: 'response' })
            .pipe(catchError(this.handleError<HttpResponse<Avatar>>('addAvatar')));
    }
    ...
    getRatingsAverage() {
        this.clearError();
        const url = `${this.baseUrl}/user/ratings-average`;
        return this.http.get<Rating>(url, { headers: this.createCacheHeaders() }).pipe(catchError(this.handleError<Rating>('getRatingsAverage')));
    }
    findWordDefinition(word: string) {
        const phpServerUrl = 'https://recherche-de-mots.herokuapp.com';
        // const phpServerUrl = 'http://localhost:80';
        const url = `${phpServerUrl}/word-definition`;
        const body = new URLSearchParams();
        body.set('motWiki', word);
        return this.http
            .post<any>(url, body.toString(), { headers: this.createHttpHeadersForWords() })
            .pipe(catchError(this.handleError<any>('findWordDefinition')));
    }
    createHttpHeadersForWords() {
