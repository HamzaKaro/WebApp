...
fdescribe('ChatComponent', () => {
    ...
    describe('addMessage() tests', () => {
        ...
        it('should not be possible to add a message composed of only "\n"', () => {
            const initialChatLength = component.chatHistory.length;
            const message: ChatMessage = { text: '\n\n\n', color: validColor, sender: SYSTEM_NAME };
            component.addMessage(message);
            expect(component.chatHistory.length).toEqual(initialChatLength);
        });
        it('should not be possible to add a message composed of only spaces', () => {
            const initialChatLength = component.chatHistory.length;
            const message: ChatMessage = { text: '   ', color: validColor, sender: SYSTEM_NAME };
            component.addMessage(message);
            expect(component.chatHistory.length).toEqual(initialChatLength);
        });
        it('should not be possible to add a message composed of only tabs', () => {
            const initialChatLength = component.chatHistory.length;
            const message: ChatMessage = { text: '          ', color: validColor, sender: SYSTEM_NAME };
            component.addMessage(message);
            expect(component.chatHistory.length).toEqual(initialChatLength);
        });
        it('should not be possible to add a message composed of only invisible characters', () => {
            const initialChatLength = component.chatHistory.length;
            const message: ChatMessage = { text: '      \n \n    ', color: validColor, sender: SYSTEM_NAME };
            component.addMessage(message);
            expect(component.chatHistory.length).toEqual(initialChatLength);
        });
        it('should add a message composed of only invisible characters and a only one printable character', () => {
            const initialChatLength = component.chatHistory.length;
            const message: ChatMessage = { text: '      n', color: validColor, sender: SYSTEM_NAME };
            component.addMessage(message);
    ...
    describe('handleSubmit() tests', () => {
        ...
        it('calling handleSubmit should clear the input when a non-empty invalid message is sent', () => {
            const message = component.chatForm.controls.message;
            message.setValue('           ');
            component.handleSubmit();
            expect(message.value).toEqual('');
        });
        it('calling handleSubmit should add an invalid message, but not send it to everyone', () => {
            const message = component.chatForm.controls.message;
